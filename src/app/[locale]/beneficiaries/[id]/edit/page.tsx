...
import { ArrowLeft, Save, User, Phone, MapPin, IdCard, AlertCircle, AlertTriangle, Search, CheckCircle } from 'lucide-react'
...
export default function EditBeneficiaryPage() {
    ...
    const loadBeneficiary = async () => {
      try {
        ...
        setFormData({
          ...
          name_ar: beneficiaryData.name_ar || '',
          age: beneficiaryData.age,
          gender: beneficiaryData.gender || 'male',
          mobile_number: beneficiaryData.mobile_number || '',
          additional_mobile_number: beneficiaryData.additional_mobile_number || '',
          email: beneficiaryData.email || '',
          alternative_contact: beneficiaryData.alternative_contact || '',
          national_id: beneficiaryData.national_id || '',
          country: beneficiaryData.country || 'Egypt',
          id_type: beneficiaryData.id_type || 'national_id',
          id_type_id: beneficiaryData.id_type_id || '',
          city: beneficiaryData.city || '',
          city_id: beneficiaryData.city_id || '',
          address: beneficiaryData.address || '',
          medical_condition: beneficiaryData.medical_condition || '',
          notes: beneficiaryData.notes || '',
          risk_level: beneficiaryData.risk_level || 'low'
        })
        
        setCitySearchQuery(beneficiaryData.city || '')
      } catch (error) {
  ...
  const handleChange = (field: keyof UpdateBeneficiaryData, value: string | number | boolean) => {
    setFormData(prev => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedBeneficiary = await BeneficiaryService.update(beneficiaryId, formData)
      router.push(`/beneficiaries/${updatedBeneficiary.id}`)
    } catch (error) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">{t('editBeneficiary') || 'Edit Beneficiary'}</h1>
          <p className="text-gray-600 mt-1">{t('editBeneficiaryDescription') || 'Update beneficiary information'}</p>
        </div>
      </div>
          ...
          >
            {isSubmitting ? (
              ...
              <>
                ...
                {t('updateBeneficiary') || 'Update Beneficiary'}
              </>
            )}
          </Button>
        </div>

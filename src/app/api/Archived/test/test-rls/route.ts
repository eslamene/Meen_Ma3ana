...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    type CaseResult = { id: string; title: string | null; status: string | null; createdBy: string | null } | { error: string }
    type ContributionResult = { id: string; amount: string | null; status: string | null; donorId: string | null } | { error: string }
    type UserResult = { id: string; email: string | null; role: string | null } | { error: string }
    
    const results: {
      user: { id: string; role: string | null; email?: string }
      accessibleCases: CaseResult[]
      accessibleContributions: ContributionResult[]
      accessibleUsers: UserResult[]
    } = {
      user: {
        id: user.id,
        role: userRole,

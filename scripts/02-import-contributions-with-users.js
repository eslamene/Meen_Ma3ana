  ...
  try {
    ...
    if (contribsWithCases && contribsWithCases.length > 0) {
      ...
      for (const contrib of contribsWithCases) {
        ...
        // Create donor notifications (both pending and approved)
        // For historical contributions, create both to show the full lifecycle
        if (contrib.donor_id) {
          // Pending notification (when contribution was submitted)
          notificationsToInsert.push({
            type: 'contribution_pending',
          ...
          // Approved notification (when contribution was approved)
          // Use a slightly later timestamp to show approval happened after submission
          const approvalDate = new Date(contrib.created_at)
          approvalDate.setMinutes(approvalDate.getMinutes() + 1) // 1 minute after submission
